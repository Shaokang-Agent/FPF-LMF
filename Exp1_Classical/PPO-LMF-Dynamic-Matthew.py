...
for round_num in range(round):
    writter = SummaryWriter("./" + config['agent']['env_name'] + "/runs/PPO-LMF-Dynamic-Matthew" + "-fed_epi" + str(fed_epi) + "/" +str(round_num))
    DPi = []
    DV = []
    ...
    for i_episode in tqdm(range(n_episode)):
                ...
                if (i_episode + 1) % fed_epi == 0 and steps == T:
                    ...
                    for i in range(n_agent):
                        ...
                        for j in [i-2, i-1, i+1, i+2]:
                            comm_gradients.append(DPi_gradients[j % n_agent])
                        DPi[i].update_consensus_gradients(comm_gradients)
        ...
        print(config['agent']['env_name'], "PPO-LMF-Dynamic-Matthew" + "-fed_epi" + str(fed_epi), round_num, i_episode)
        print(su, su.sum())
        writter.add_scalar("Total Rewards", su.sum(), i_episode)
        reward_step[round_num, i_episode] = su
        np.save("./" + config['agent']['env_name'] + "/PPO-LMF-Dynamic-Matthew" + "-fed_epi" + str(fed_epi) + ".npy", reward_step)
