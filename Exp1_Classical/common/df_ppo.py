...
class PPOPolicyNetwork():
    ...
    def broad_gradients(self):
        return self.gradients

    def update_consensus_gradients(self, DP_gradients):
        for gradient in DP_gradients:
            self.session.run(self.apply_grad, feed_dict={
                self.W0_grad: gradient[0][0],
                self.W1_grad: gradient[1][0],
                self.W2_grad: gradient[2][0],
                self.B0_grad: gradient[3][0],
                self.B1_grad: gradient[4][0],
                self.B2_grad: gradient[5][0],
            })

    def update_comm_parameters(self, other_parameters):
        dict = ["W1:0", "W2:0", "W3:0", "B1:0", "B2:0", "B3:0"]
        for name in dict:
            local_parameters = self.session.run(self.tf_graph.get_tensor_by_name(name))
            local_parameters = (1 - 0.1) * local_parameters
            for other_parameter in other_parameters:
                param = other_parameter[name]
                local_parameters = local_parameters + 0.1 * param / len(other_parameters)
            update = tf.assign(self.tf_graph.get_tensor_by_name(name), local_parameters)
            self.session.run(update)
