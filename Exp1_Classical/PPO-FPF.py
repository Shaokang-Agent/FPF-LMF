...
for round_num in range(round):
    writter = SummaryWriter("./" + config['agent']['env_name'] + "/runs/PPO-FPF-w" + str(lamdaw) + "-e" + str(lamdae) + "-fed_epi" + str(fed_epi) + "/" +str(round_num))
    DPi = []
    DV = []
    ...
    for i_episode in tqdm(range(n_episode)):
        ...
        while steps < max_steps and not done:
            ...
            if steps % T == 0:
                ...
                if (i_episode + 1) % fed_epi == 0 and steps == T:
                    omega_ego_models = []
                    omega_uti_model = FutiPi.broad_parameters()
                    omega_ega_model = FegaPi.broad_parameters()
                for i in range(n_agent):
                    ...
                    if (i_episode + 1) % fed_epi == 0 and steps == T:
                        ...
                        if i <= n_agent // 2:
                            omega_ego_models.append(DPi[i].broad_parameters())
                if (i_episode + 1) % fed_epi == 0 and steps == T:
                    kwargs = {}
        ...
        print(config['agent']['env_name'], "PPO-FPF-w" + str(lamdaw) + "-e" + str(lamdae) + "-fed_epi" + str(fed_epi), round_num, i_episode)
        print(su, su.sum())
        writter.add_scalar("Total Rewards", su.sum(), i_episode)
        reward_step[round_num, i_episode] = su
        np.save("./" + config['agent']['env_name'] + "/PPO-FPF-w" + str(lamdaw) + "-e" + str(lamdae) + "-fed_epi" + str(fed_epi) + ".npy", reward_step)
