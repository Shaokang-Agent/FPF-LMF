...
for round_num in range(round):
    writter = SummaryWriter("./" + config['agent']['env_name'] + "/runs/PPO-LMF" + "-fed_epi" + str(fed_epi) + "/" +str(round_num))
    DPi = []
    DV = []
    ...
    for i_episode in tqdm(range(n_episode)):
        ...
        while steps < max_steps and not done:
            ...
            if steps % T == 0:
                ...
                if (i_episode + 1) % fed_epi == 0 and steps == T:
                    ...
                    for i in range(n_agent):
                        DPi[i].update_consensus_gradients(DPi_gradients[:i] + DPi_gradients[i+1:])
        ...
        print(config['agent']['env_name'], "PPO-LMF" + "-fed_epi" + str(fed_epi), round_num, i_episode)
        print(su, su.sum())
        writter.add_scalar("Total Rewards", su.sum(), i_episode)
        reward_step[round_num, i_episode] = su
        np.save("./" + config['agent']['env_name'] + "/PPO-LMF" + "-fed_epi" + str(fed_epi) + ".npy", reward_step)
