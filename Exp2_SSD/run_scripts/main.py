...
from run_scripts.runner_fpf import Runner_fpf
from run_scripts.runner_lmf import Runner_lmf


parser = argparse.ArgumentParser()

parser.add_argument("--algorithm", type=str, default="FPF") #FPF,LMF,DQN,DQN-AVG,DQN-MIN,DQN-RMF,DQN-IA,SOCIAL,QMIX,PPO,MAPPO,DDPG,MADDPG
...
if args.algorithm != "PPO" and args.algorithm != "MAPPO" and args.algorithm != "FPF" and args.algorithm != "LMF":
...
if args.algorithm == "FPF":
    ...
    print("Initialized: FPF")
    print("Environment: {}, agent-number: {}, algorithm: {}, cuda: {}".format(args.env, args.num_agents, args.algorithm, args.cuda))
    runner = Runner_fpf(args)
    for i in range(args.round):
        runner.run(i)

if args.algorithm == "LMF":
    args.gamma = 0.95
    args.lr = 1e-3
    args.epsilon_init = 0.5
    args.epsilon_steplen = args.num_episodes/5
    ...
    args.fed_epis = 10
    args.double_dqn = True
    print("Initialized: LMF")
    print("Environment: {}, agent-number: {}, algorithm: {}, cuda: {}".format(args.env, args.num_agents, args.algorithm, args.cuda))
    runner = Runner_lmf(args)
    for i in range(args.round):
        runner.run(i)
