...
class D_Agent:
    ...
    def upload_parameters(self, omega_federation_model):
    ...
    def broad_gradients(self):
        ...
        with torch.no_grad():
            for name, param in self.policy.actor_network.named_parameters():
                actor_grad[name] = param.grad
            for name, param in self.policy.critic_network.named_parameters():
                critic_grad[name] = param.grad
        return actor_grad, critic_grad

    def update_consensus_gradients(self, actor_grads, critic_grads):
        for actor_grad in actor_grads:
            self.policy.actor_optim.zero_grad()
            for name, param in self.policy.actor_network.named_parameters():
                param.grad = actor_grad[name]
            self.policy.actor_optim.step()


        for critic_grad in critic_grads:
            self.policy.critic_optim.zero_grad()
            for name, param in self.policy.critic_network.named_parameters():
                param.grad = critic_grad[name]
