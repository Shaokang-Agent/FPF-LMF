...
from Agent.df_agent import D_Agent
...
class Runner:
    ...
    def _init_agents(self):
        ...
        return d_agents, adv_agent

    def run(self, run_number):
        self.d_agents, self.adv_agent = self._init_agents()
            ...
            for step in range(self.args.max_episode_step):
                ...
                if self.buffer.current_size >= self.args.batch_size:
                    ...
                    if (step + 1) % self.args.fed_steps == 0:
                        ...
                        for agent in self.d_agents:
                            actor_grad, critic_grad = agent.broad_gradients()
                            actor_gradients.append(actor_grad)
                            critic_gradients.append(critic_grad)
                        for i, agent in enumerate(self.d_agents):
                            agent.update_consensus_gradients(actor_gradients[:i] + actor_gradients[i + 1:], critic_gradients[:i] + critic_gradients[i + 1:])
    ...
    def evaluate(self, run_number):
        ...
        for episode in range(self.args.evaluate_episodes):
            ...
            print('Algorithm {}, Run {}, Returns is {}'.format(self.args.algorithm, run_number, rewards))
